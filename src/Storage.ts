...
export class PartitionedStorage<T extends object> implements Storage<Boxed<T>> {
  ...
  constructor(
    ...
    public partitioner: (value: T) => number
  ) {
    this.partitions = []
  }
  ...
  update(index: number, oldValue: T | undefined, newValue: T | undefined): void {
    if (oldValue !== undefined) {
      const oldPartitionIndex = this.partitioner(oldValue)
    ...
    if (newValue !== undefined) {
      const newPartitionIndex = this.partitioner(newValue)
  ...
  getPartitionByIndex(index: number): number[] {
    return this.partitions[index]
  }
